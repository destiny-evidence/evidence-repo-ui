...
// the `start-end` year range — feeding one `Filters / Applied` event each;
// `categories` is the facet each belongs to (the concept scheme uri, since
// schemes have no separate id, plus "country" and "year-range"), feeding one
// `Filters / Category Applied` event each. Both are emitted because Matomo can't
// roll values up to their category itself. Concept uris outside any supplied
// scheme are dropped from both.
